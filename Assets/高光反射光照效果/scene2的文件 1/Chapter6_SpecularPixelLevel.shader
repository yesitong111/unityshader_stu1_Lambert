    ...
    SubShader
    {
        Pass{
            ...
            fixed4 frag(v2f i) : SV_Target//片元着色器，输入为顶点着色器传递的结构体v2f，输出为一个四分量颜色（RGBA），使用SV_Target语义表示这是渲染目标的输出。
            {
                ...
                fixed3 specular = _LightColor0.rgb * _Specular.rgb * pow(saturate(dot(reflectDir, viewDir)), _Gloss);// pow(saturate(dot(reflectDir, viewDir)), _Gloss)高光强度系数，这部分算出一个0~1的倍率，用来控制高光的亮暗和范围。dot(reflectDir, viewDir)：计算高光的基础亮度：两个向量都已经归一化为单位向量，点积结果就是它们夹角的余弦值：反射光方向和视线方向完全重合时，夹角为0°，点积=1，高光最亮；两者夹角越大，点积越小，高光越暗；夹角＞90°时点积为负数，对应背光面，理论上看不到高光。pow(...,_Gloss):控制高光的锐利程度。pow是内置的乘方函数。_Gloss（光泽度/高光指数）是自定义材质参数，面板可调
                //入射光颜色亮度 x 材质自身高光属性 x 视角决定的高光强度 = 这个像素最终呈现的高光颜色。specular 是一个 RGB 三分量的颜色值，它同时包含了高光的色调和亮度，就是当前像素高光分量的最终计算结果。
