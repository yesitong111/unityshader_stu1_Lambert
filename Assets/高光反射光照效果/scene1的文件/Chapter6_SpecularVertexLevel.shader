...
Shader "Unity Shaders Book/Chapter6/SpecularVertexLevel"
{
    ...
    SubShader
    {
        ...
        v2f vert(a2v v){
            v2f o;
            0.pos = UnityObjectToClipPos(v.vertex);//mul()是一个函数，用于执行矩阵乘法运算。在这里，它将顶点位置v.vertex与UNITY_MATRIX_MVP矩阵相乘，得到变换后的顶点位置o.pos。UNITY_MATRIX_MVP是一个内置的矩阵，包含了模型、视图和投影矩阵的组合，用于将顶点从模型空间转换到裁剪空间。
            ...
            fixed3 worldNormal = normalize(mul(v.normal,(float3x3)unity_WorldToObject));//将顶点的法线向量v.normal从对象空间转换到世界空间。首先，将v.normal与_World2Object矩阵相乘，得到在世界空间中的法线向量。然后，使用normalize函数对结果进行归一化，以确保法线向量的长度为1。
            ...
            fixed3 viewDir = normalize(_WorldSpaceCameraPos.xyz - mul(unity_ObjectToWorld, v.vertex).xyz);//计算视线方向。首先，将顶点位置v.vertex从对象空间转换到世界空间，得到顶点在世界空间中的位置。然后，使用_WorldSpaceCameraPos表示摄像机在世界空间中的位置，计算摄像机位置与顶点位置之间的向量。最后，使用normalize函数对结果进行归一化，以获得一个单位长度的视线方向viewDir。
