...
Shader "Unity Shaders Book/Chapter 6/DiffusePixelLevel"
{
    ...
    SubShader
    {
        Pass{
            ...
            {
                v2f o;
                // 定义顶点着色器的输入a2v v（存储顶点位置、法线等模型空间的顶点数据），输出结构体v2f 0（存储要传递给片元着色器、后续会被插值的数据）
            ...
            fixed4 frag(v2f i) : SV_Target//片元着色器，输入为顶点着色器传递的结构体v2f，输出为一个四分量颜色（RGBA），使用SV_Target语义表示这是渲染目标的输出。
            {
                ...
                fixed3 diffuse = _LightColor0.rgb * _Diffuse.rgb * saturate(dot(worldNormal, worldLightDir));//计算漫反射分量，_LightColor0是Unity内置的一个变量，表示主光源的颜色，_Diffuse.rgb是材质的漫反射颜色，dot(worldNormal, worldLightDir)计算法线与光照方向的点积得到入射角的余弦值，saturate函数将结果限制在0到1之间，确保当光线背向表面时漫反射分量为0。
                //四者相乘得到该像素的漫反射光颜色。
...
/*逐顶点漫反射是在顶点着色器中计算每个顶点的光照颜色，再对图元内部的顶点颜色做插值，得到每个像素的最终颜色，它的效果高度依赖模型的顶点密度。
逐像素漫反射将光照计算移到片元着色器中，对每个像素独立完成光照计算，光影过渡更平滑自然，光影边界更柔和。*/
